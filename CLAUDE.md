...
- **보충 슬라이드는 재료가 남는 만큼 늘어난다.** `_extra_slides` 는 수치·조건·용어를 4개씩
  끊어 여러 장으로 만들고 종류를 번갈아 내보낸다. 한때는 종류당 한 장이라 보충이 최대 3장이었고,
  원문에 수치가 26개 있어도 10장을 요청한 덱이 8장에서 멈췄다 — 화면은 고른 장수를 그대로
  예고하므로 모자라면 그 예고가 거짓말이 된다. 나눠 담는 것이라 사실을 만들지 않고
  `source_refs` 도 그대로다. 여러 장이 되면 제목에 `(2/4)` 를 붙이고 결론 문장은 그 장이
  실제로 담은 원문 문장으로 바꾼다 — 같은 문장이 네 장에 붙으면 한 장을 복사한 것으로 보인다.
  `tests/test_planner.py::test_requested_slide_count_is_honored` 가 지킨다.
...
- **원본 위에 얹을 때 갈아 끼우지 않은 텍스트 상자는 그대로 둔다.** 한때는 비웠다. 검증하지
  않은 원문이 결과에 섞이는 것을 막으려는 것이었는데, 실제 자료를 넣어 보니 차트의 축 이름·
  수치 라벨·범례·각주가 통째로 사라져 발표에 못 쓰는 파일이 나왔다(원본 슬라이드 하나에
  텍스트 상자가 40~90개인 IR 자료가 흔하다). 바꾸는 것은 제목 자리와 본문 자리 둘뿐이고,
  나머지는 원본 그대로다. 원문 대비 검증은 화면의 검증 탭이 계속 책임진다.
- **제목 placeholder 가 없으면 위쪽 띠에서 글자가 가장 큰 상자를 제목 자리로 본다.** 빈 레이아웃
  위에 직접 만든 자료가 흔한데, `shapes.title` 이 None 이면 생성된 제목이 어디에도 안 들어가고
  원본 제목이 그대로 남는다. 한때는 **맨 위** 상자를 썼다. 실무 자료의 맨 위 한 줄은 거의 항상
  머리글이라("Financials > 2Q26 손익현황", 오른쪽 구석의 문서명, 쪽번호) 12pt 머리글이 발표
  제목으로 바뀌고 25pt 원본 제목은 그대로 남아, 한 장에 제목이 둘이 됐다. 글자 크기가 제목과
  머리글을 가르는 가장 곧은 신호다. 띠는 위 20% 로 좁게 잡는다 — 넓히면 제목 바로 아래의
  45pt 실적 숫자("92,929억원")가 제목으로 뽑혀 원본에서 통째로 사라진다. 고른 상자가 제목
  한 줄이라기엔 너무 높으면(제목·본문이 한 상자) 본문 자리로 넘긴다.
- **본문 자리는 "가장 넓은 상자"가 아니다.** IR 자료 한 장에는 텍스트 상자가 40~90개 있고
  대부분이 차트 축 이름·수치 라벨·범례·각주다. 면적만 보면 눈에 안 보이는 빈 장식 상자나
  '92,929억원' 같은 라벨이 뽑혀, 원본 숫자가 생성 문장으로 덮이고 정작 본문은 아무도 못 보는
  자리에 처박힌다. 통과시키는 것은 둘뿐이다 — 원본이 본문 자리로 설계한 placeholder, 그리고
  문장이 든 충분히 큰 상자(폭 30%·높이 10% 이상). 고른 상자에 글이 바닥 크기로도 안 들어가면
  쓰지 않는다. 셋 다 없으면 빈 구간을 찾고, 그것도 없으면 **제목만 바꾸고 본문은 얹지 않는다** —
  도표로 꽉 찬 장에 억지로 넣으면 발표에 못 쓰는 파일이 된다.
...
- **원본 위에 얹을 때는 표지를 남기고 부록을 붙이지 않는다.** 완성본은 원본 슬라이드로만
  이루어지고 장수가 원본을 넘지 않는다. 새로 그린 표지도, 예상 Q&A·검증 부록도 붙이지
  않는다 — 10장을 넣고 10장을 요청한 사람에게 14장을 주면 그 파일은 발표에 그대로 못 쓴다.
  Q&A 와 검증은 화면과 Markdown·JSON 다운로드에 있다. 원본 1장(표지)은 글까지 손대지 않고
  맨 앞에 남기고, 생성 슬라이드는 나머지 원본에 얹는다(`_assign_sources`). 배치 순서는
  **덱 순서**다 — 원본 순서로 되돌리면 청중별로 다시 짠 이야기 순서가 파일에서 사라진다.
  원본 없이 새로 그리는 경로(PDF·TXT 입력)는 예전대로 표지와 부록을 붙인다.
- **짝짓기의 원본은 `export_pptx` 하나이고, 화면은 `GET /api/presentations/{id}/source-map`
  으로 물어본다.** 대조 화면이 같은 규칙을 옮겨 적던 시절에는 표지를 후보에서 빼는 것도, 짝을
  못 찾은 슬라이드를 남은 원본에 채우는 것도 export 만 알고 있어, 원본 2장에 얹힌 슬라이드를
  원본 1장(표지) 옆에 놓았다. 파일 쪽수도 여기서 온다 — 원본 장수가 모자라면 뒤쪽 덱 슬라이드가
  빠지므로 "덱 N 장 = 파일 N+1 장"이 아니다. 원본이 PPTX 가 아니면 얹을 원본이 없어 짝도 없다.
  `SourceCompare` 의 "근거로 짝을 지어 글끼리 견주는" 갈래는 이제 짝짓기를 못 받았을 때의
  대비책으로만 남는다.
- **"원본과 비교" 버튼은 업로드가 PPTX 일 때만 눌린다.** PDF·TXT 는 맞댈 원본 슬라이드가 아예
  없어 열어 봐야 왼쪽이 글 뭉치뿐이다. 누르게 두고 빈 화면으로 실망시키는 대신 버튼에서 막고,
  버튼에 "PPTX 전용" 라벨을 달아 색만으로 꺼진 것을 알리지 않으며, 왜 안 되는지와 대신 어디를
  보면 되는지(정확성 검증 탭)를 바로 아래 문장에 적고 `aria-describedby` 로 버튼에 매단다.
  판정은 `ResultView` 의 `canCompare` 한 곳이다.
